...
const HeaderComp = () => {
    ...
    // ----- Desktop layout (unchanged original AntD grid) -----
    const searchCol = isAuthenticated ? 17 : 20
    const authField: FieldConfig = isAuthenticated
        ? { col: 5, other: <HeaderProfile /> }
        : {
            col: 2,
            button: {
                label: 'Login',
    ...
    return (
        <div className="sticky top-0 z-50 w-full backdrop-blur-md bg-black/30 border-b border-white/10">
            {/* Desktop (lg and up): original layout, unchanged */}
            <div className="hidden lg:block max-w-7xl mx-auto p-4">
                <FormApp config={config_header} form={form_header} />
                <div className="mt-5"></div>
                <FormApp config={config_header_2} form={form_header} />
            </div>

            {/* Mobile / tablet (below lg): responsive flex layout */}
